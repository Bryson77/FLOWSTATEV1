  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (upcoming) {
        ...
        const dayIndex =
          typeof first.day_of_week === "number" ? first.day_of_week : 1;
        setNextClass({
      ...
      setErrorMsg(
        getSafeErrorMessage(
          err,
          "Failed to load study data. Something went wrong.",
        ),
      );
    } finally {
      setLoading(false);
        ...
        <View style={styles.metricsGrid}>
          <View style={styles.metricCard}>
            ...
            <View style={styles.metricValueRow}>
              <Text style={styles.metricValue}>{todayStudyMinutes}m</Text>
              <Text style={styles.metricTarget}>
                / {profile?.daily_study_goal_minutes || 120}m
              </Text>
            </View>
          ...
          <View style={styles.metricCard}>
            <View style={styles.metricHeader}>
              ...
              <Text style={styles.metricLabel}>CARDS DUE</Text>
            </View>
            <Text style={styles.metricValue}>{cardsDueCount}</Text>
            <Text style={styles.metricSub}>
              {cardsDueCount > 0 ? "Review queue" : "Caught up"}
            </Text>
          </View>
          ...
          <View style={styles.metricCard}>
            ...
            <Text style={styles.metricValue}>{assessments.length}</Text>
            <Text style={styles.metricSub}>
          ...
          <View style={styles.metricCard}>
            ...
            <Text style={styles.metricSub}>
              {tasks.length > 0
                ? `${Math.round((tasks.filter((t) => t.done).length / tasks.length) * 100)}% done`
                : "Zero tasks"}
            </Text>
          </View>
        </View>

        {/* Flashcards Due (Active Recall) - Open Cards Button */}
          ...
          <View style={styles.taskList}>
            {tasks.length === 0 ? (
              ...
              tasks.map((t) => (
                <View
                  key={t.id}
                  style={[styles.taskRow, t.done && styles.taskDone]}
                >
                  <Pressable
                  ...
                  <Text
                    style={[styles.taskText, t.done && styles.taskStrike]}
                    numberOfLines={2}
                  >
                    {t.text}
                  </Text>
              ...
              return (
                <View key={exam.id} style={styles.assessmentRow}>
                  <View style={{ flex: 1, minWidth: 0, paddingRight: 8 }}>
                    ...
                    <Text style={styles.examSub} numberOfLines={1}>
                      ...
                      {exam.weight_percentage
                        ? ` · ${exam.weight_percentage}% of mark`
                        : ""}
                  ...
                  <View
                    style={[
                      styles.daysPill,
                      isDueSoon && styles.daysPillUrgent,
                    ]}
                  >
                    <Text
                      style={[
                        styles.daysText,
                        isDueSoon && styles.daysTextUrgent,
                      ]}
                    >
